#!/bin/bash
# Batch segmentation test for all PQFC DINOv2 checkpoints.
set -euo pipefail
...
LOG_DIR="$PQFC_DIR/logs/test_seg"
mkdir -p "$LOG_DIR"
NUM_GPUS="${NUM_GPUS:-4}"
...
TOTAL=${#JOBS[@]}
[ "$TOTAL" -gt 0 ] || { echo "No DINOv2 checkpoints in $WEIGHTS_DIR"; exit 1; }

echo "PQFC Segmentation Test — $TOTAL jobs, GPUs=$NUM_GPUS"
