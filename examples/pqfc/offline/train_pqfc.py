#!/usr/bin/env python
"""
PQFC Training — Product Quantization Feature Codec.

Orthogonal transform is optional:
  --use_transform (default): OrthogonalTransform + OPQ warm-start + soft PQ
                             (aligned with examples/orfc_2446)
  --no_transform:            no R; k-means init; hard PQ (tau_start forced to 0)

Loss: J = R + λ · D, where D is frozen-tail ΔL_ref (not feature-space MSE).

Usage:
    python train_pqfc.py --backbone dinov2_vitl14 --layer blk10 \
        --K 64 --embedding_dim 32 --epochs 100
...
from cofai.entropy_models.soft_pq import (
    OrthogonalTransform, FeatureTransform,
...
def _apply_transform_tau_binding(args):
    """Bind soft/hard assignment to orthogonal transform switch.

    use_transform=True  -> soft PQ (orfc_2446 defaults: 0.5 -> 0.005)
    use_transform=False -> hard PQ (tau_start forced to 0)
    """
...
def train_and_save(args):
    ...
    print(f"# PQFC Training ({mode_str})")
    ...
    print(f"# use_transform={args.use_transform}, transform_params: {freeze_str}")
    tau_info = f", τ={args.tau_start}→{args.tau_end}" if args.tau_start > 0 else ", hard PQ"
    ...
    Dp = bt_dim if args.use_transform else D
    ...
    # ---- Load backbone (for frozen tail / ΔL_ref) ----
    ...
    R_std = codebooks_std = None
    opq_usage_counts = None

    if args.use_transform:
        # ---- OPQ Baseline (for warm-start) — aligned with orfc_2446 ----
        print(f"\n{'=' * 60}")
        ...
        print(f"\n{'=' * 60}")
        print(f"  [init] no orthogonal R; k-means in train_soft_pq")
        print(f"{'=' * 60}")
        wrapper.backbone.cpu()
        if wrapper.head is not None:
            wrapper.head.cpu()
    ...
    if args.use_transform:
        ...
        if args.warm_start_opq and bt_dim == D and R_std is not None:
            R_ws = R_std.copy()
    ...
    # ---- Train codec ----
    print(f"\n{'=' * 60}")
    tau_str = f", τ={args.tau_start}→{args.tau_end}" if args.tau_start > 0 else ", hard"
    ws_str = "warm-start" if (args.use_transform and args.warm_start_opq) else "k-means"
...
def main():
    parser = argparse.ArgumentParser(
        description="PQFC training — save codec checkpoint (.pt)",
    ...
    parser.add_argument("--use_transform", action="store_true", default=True,
                        help="Learn orthogonal R + soft PQ (default, orfc_2446-aligned)")
    parser.add_argument("--no_transform", dest="use_transform", action="store_false",
                        help="No orthogonal R; hard PQ (tau_start forced to 0)")

    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--lmbda", type=float, default=0.5,
                        help="R-D Lagrange multiplier: J = R + λ·D")
    parser.add_argument("--tau_start", type=float, default=0.5,
                        help="Initial soft-PQ temperature (overridden by transform binding)")
    ...
    parser.add_argument("--warm_start_opq", action="store_true", default=True,
                        help="Warm-start from OPQ (only when use_transform)")
