...
LOG_DIR="examples/orfc_2446_dinov3/logs/analyze"
mkdir -p "$LOG_DIR"
...
for i in "${!TAGS[@]}"; do
  ...
  $PYTHON examples/orfc_2446_dinov3/scripts/compare_ptpatch_mse.py \
...
for i in "${!CKPTS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=$gpu \
  $PYTHON examples/orfc_2446_dinov3/scripts/analyze_token_rmse.py \
...
results = Path("examples/orfc_2446_dinov3/results")
