"""Offline differentiable Soft-PQ training for the ORFC-2446 proposal.

This module is intentionally proposal-local. The stable online codec lives in
``cofai.latent_codecs.orfc`` and only consumes exported ``.npz`` artifacts.
...
from cofai.ops.orfc import batched_kmeans
...
class OrthogonalTransform(nn.Module):
    ...
    def _get_skew(self):
        A = torch.zeros(self.D, self.D, device=self.triu_params.device, dtype=self.triu_params.dtype)
...
class SoftPQ(nn.Module):
    ...
    def init_from_kmeans(self, Z_flat, device="cuda", max_iter=100):
        ...
        del Z_flat
        with torch.no_grad():
            centroids = batched_kmeans(sub, self.K, max_iter=max_iter, device=device, verbose=False)
...
def train_soft_pq(
        ...
        if precompute_teacher:
            ...
            with torch.no_grad():
                for start in range(0, N_img, batch_size):
                    ...
                    X_chunk = torch.from_numpy(features_train[start:end]).float().to(device)
                    teacher_cache[start:end] = tail.forward_nograd(X_chunk).cpu().numpy()
                    del X_chunk
            torch.cuda.empty_cache()
            if verbose:
                cache_gb = teacher_cache.nbytes / 1e9
                print(f"  Teacher cache: {cache_gb:.1f} GB CPU " f"({time.time() - t_pre:.1f}s)")
        elif verbose:
    ...
    if val_features is not None and len(val_features) > 0:
        if isinstance(val_features, np.ndarray):
            ...
            val_array = np.stack([_load_feature_item(val_features, i) for i in range(len(val_features))])
        else:
        ...
        if not use_mse_loss and precompute_teacher:
            ...
            with torch.no_grad():
                for vs in range(0, n_val, batch_size):
                    ...
                    val_teacher_cache[vs:ve] = tail.forward_nograd(X_v).cpu().numpy()
                    del X_v
            torch.cuda.empty_cache()
    ...
    for epoch in range(epochs):
        ...
        if val_array is not None:
            ...
            with torch.no_grad():
                for vs in range(0, n_val, batch_size):
                    ...
                    X_v = torch.from_numpy(val_array[vs:ve]).float().to(device)
                    Bv = X_v.shape[0]
                    Y_v, Mu_v, Std_v = normalize_orfc_features(
                        X_v,
                        mode=norm_mode,
                        n_prefix=n_prefix,
                    )
                    ...
                    if use_mse_loss:
                        ...
                        if val_teacher_cache is not None:
                            Yt_v = torch.from_numpy(val_teacher_cache[vs:ve]).float().to(device)
                        else:
                            Yt_v = tail.forward_nograd(X_v)
                        Xh_v = denormalize_orfc_features(
                            Yh_v,
                            Mu_v,
                            Std_v,
                            mode=norm_mode,
        ...
        if verbose and (epoch % 10 == 0 or epoch == epochs - 1):
            ...
            rate_str = ""
            if codec.use_rate:
                rate_str = f"  R={rate_bits_bpt:.2f}b/t" f"  lD={lmbda * avg_distortion:.1f}" f"  dead={dead_entries}"
...
def soft_pq_encode_decode(features, codec, norm_mode, device, chunk_images=None, n_prefix=0):
...
def save_codec(codec, path, **extra_meta):
    ...
    meta = {
        ...
        "transform_type": (type(codec.transform).__name__ if codec.transform else None),
    }
...
def load_codec(path, device="cuda"):
    ...
    pq = SoftPQ(G, K, d, lmbda=meta.get("lmbda", 0.0), prior_floor=meta.get("prior_floor", 0.0))
    transform = None
