...
def train_and_save(args):
    ...
    npz_path = save_codec_npz(
        codec,
        os.path.join(out_dir, f"{ckpt_name}.npz"),
        pmf,
        source_pt=ckpt_path if getattr(args, "keep_pt", False) else None,
        norm_mode=args.norm_mode,
        n_prefix=getattr(args, "n_prefix", 0),
    )
    print(f"  Official NPZ saved: {npz_path}")
    ...
    if getattr(args, "keep_pt", False):
        print(f"  Resume PT saved: {ckpt_path}")
