...
class CodecSegmentationEvaluator(SegmentationEvaluator):
    """SegmentationEvaluator with SoftPQFeatureCodec (.npz) encode/decode."""
    ...
    def quantize_tokens(self, tokens_np):
        ...
        # SoftPQFeatureCodec applies norm + PQ + denorm internally
...
def softpq_rate_bits(features, latent, device, chunk_images=64):
    latent.eval()
    total_bits = 0.0
    total_tokens = 0
    with torch.no_grad():
...
def run_test(args):
    ...
    codec_seg = CodecSegmentationEvaluator(
        codec=latent, norm_mode=args.norm_mode,
        layer_idx=layer_idx,
    ...
    if image_features_list:
        total_bits, total_tokens = softpq_rate_bits(
            image_features_list, latent, device, chunk_images=1,
        )
        if total_tokens > 0:
