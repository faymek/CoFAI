...
from cofai.entropy_models.soft_pq_export import load_softpq_npz, pmf_as_list
...
def softpq_reconstruct(features, latent, device, batch_size=32):
    """Reconstruct features with SoftPQFeatureCodec (.npz)."""
    latent.eval()
    outs = []
    with torch.no_grad():
        for start in range(0, len(features), batch_size):
            ...
            h_hat = latent(X)["h_hat"]
            for i in range(h_hat.shape[0]):
                outs.append(h_hat[i].cpu().numpy())
            del X, h_hat
    return outs


def softpq_rate_bits(features, latent, device, chunk_images=200):
    """Sum real (or theoretical) bits over features using SoftPQFeatureCodec."""
    latent.eval()
    total_bits = 0.0
    total_tokens = 0
    with torch.no_grad():
        for start in range(0, len(features), chunk_images):
            ...
            X = torch.from_numpy(np.stack(features[start:end])).float().to(device)
            B, T, _ = X.shape
            _, _, labels = latent._assign_labels(X)
            # labels: (G, B*T) — bill per image for rANS when pmf present
            for img_idx in range(B):
                lab = labels[:, img_idx * T:(img_idx + 1) * T]
                total_bits += latent._actual_bits(lab)
                total_tokens += T
            del X, labels
...
def _codec_labels(features, codec, norm_mode, device, batch_size=32):
    """Legacy helper (FeatureCodec); prefer SoftPQFeatureCodec._assign_labels."""
    codec.eval()
    pq = codec.pq
    all_labels = []
    with torch.no_grad():
...
def resolve_ckpt_path(args):
    """Resolve checkpoint path from explicit --ckpt_path or from naming / glob."""
    if args.ckpt_path:
        path = args.ckpt_path
        if path.endswith(".pt"):
            npz = path[:-3] + ".npz"
    ...
    weights_dir = codec_weights_dir(args.weights_dir, args.backbone)
    # Prefer exact training-name match when hyperparams are fully specified
    ...
    exact = os.path.join(weights_dir, ckpt_name)
    if os.path.exists(exact):
        return exact

    # Fallback: unique (or preferred) match by layer/K/emb
    pattern = f"{args.layer}_K{args.K}_emb{args.embedding_dim}_*.npz"
    matches = sorted(Path(weights_dir).glob(pattern))
    if not matches:
        raise FileNotFoundError(
            f"No SoftPQ .npz for {pattern} under {weights_dir}. "
            "Pass --ckpt_path or unzip release weights."
        )
    if len(matches) == 1:
    ...
    raise FileNotFoundError(
        "Ambiguous SoftPQ weights; pass --ckpt_path. Candidates:\n  - "
...
def run_test(args):
    ...
    if str(ckpt_path).endswith(".pt"):
        ckpt_path = str(Path(ckpt_path).with_suffix(".npz"))
    print(f"\n  Loading codec: {ckpt_path}")
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")
    payload = load_softpq_npz(ckpt_path)
    latent = SoftPQFeatureCodec(
        codec_path=ckpt_path,
        norm_mode=getattr(args, "norm_mode", None) or payload["norm_mode"],
        n_prefix=getattr(args, "n_prefix", None),
    ).to(device)
    ...
    if pmf_np is not None:
        ...
        print("  [warn] No PMF in npz; rate uses theoretical bits")
    ...
    xhat = softpq_reconstruct(features_test, latent, device)
    ...
    rate_info = None
    total_bits, total_tokens = softpq_rate_bits(features_test, latent, device)
    if total_tokens > 0:
        rans_per_image_bpt = total_bits / total_tokens
        rans_per_image_bpfp = total_bits / (total_tokens * D)
        print(f"  * per-image rate: {rans_per_image_bpt:.4f} bits/token, "
