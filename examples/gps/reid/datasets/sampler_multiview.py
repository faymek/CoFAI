...
class MultiViewSampler(Sampler):
    ...
    def __init__(self, data_source, batch_size: int, num_instances: int = 3):
        ...
        if self.batch_size % self.num_instances != 0:
            raise ValueError("batch_size must be a multiple of num_instances."
        ...
        for pid in self.pids:
            ...
            it = iter(idxs)
            self.pid_to_chunks[pid] = [list(itertools.islice(it, self.num_instances))
                                       for _ in range(len(idxs) // self.num_instances)]
        self.length = sum(len(chunks) * self.num_instances for chunks in self.pid_to_chunks.values())


    def __iter__(self):

        pid_to_chunks = {pid: chunks.copy() for pid, chunks in self.pid_to_chunks.items()}
