...
def load_checkpoint(checkpoint_path):
    ...
    state_dicts = {}
    iteration = None
    if isinstance(checkpoint, dict) and "iteration" in checkpoint.keys():
        iteration = checkpoint["iteration"]
    state_dicts["model"] = unwrap_ddp_state_dict(_extract_model_state_dict(checkpoint))
    if isinstance(checkpoint, dict) and "optimizer" in checkpoint.keys():
