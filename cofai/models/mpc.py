...
@register("MPC_I3")
class MPC_I3(CompressionModel):
    """
    Multi-Purpose Compression model using DINOv3 backbone only.

    Compared with MPC_I2 (DINOv2), DINOv3 backbones output extra prefix tokens
    (CLS + REG) besides patch tokens. This model always keeps all prefix tokens and
    compresses them as one branch, while patch tokens are compressed as another branch
    by ``VitSeparateLatentCodec``.

    Args:
        dino_backbone (dict): config for Dinov3TimmBackbone (or any compatible backbone).
        dino_codec (dict): config for (Vbr)VitSeparateLatentCodec.
        **kwargs: unused.
    """
    ...
    def forward(self, x, qp=0, **kwargs):
        with torch.no_grad():
            ...
            n_spatial = int(token_res[0]) * int(token_res[1])
            if int(h_dino.shape[1]) < n_spatial:
                raise ValueError(
                    f"DINO token length smaller than H*W: N={h_dino.shape[1]} < {n_spatial}."
                )
            n_prefix = int(h_dino.shape[1]) - n_spatial
            if n_prefix < 1:
                raise ValueError(f"Invalid prefix token count: {n_prefix}.")
            if hasattr(self.dino_codec, "num_prefix_tokens"):
                self.dino_codec.num_prefix_tokens = int(n_prefix)
            o_dino = self.dino.decode_whole(h_dino)[-1]

        dino_out = self.dino_codec(h_dino.clone(), token_res, qp=qp)
        h_dino_hat = dino_out["h_hat"]
        if isinstance(h_dino_hat, torch.Tensor) and h_dino_hat.dim() == 3:
            n_expected = n_prefix + n_spatial
            if int(h_dino_hat.shape[1]) != n_expected:
                raise RuntimeError(
    ...
    def compress(self, x, qp=0, **kwargs):
        ...
        coded_unit["pstate"]["n_prefix_expected"] = int(n_prefix)
        return coded_unit

    def decompress(self, coded_unit, tasks=[], **kwargs):
        token_res = coded_unit["pstate"]["token_res"]
        n_spatial = int(token_res[0]) * int(token_res[1])
        n_prefix = int(coded_unit["pstate"].get("n_prefix_expected", getattr(self.dino.model, "num_prefix_tokens", 1)))
        if hasattr(self.dino_codec, "num_prefix_tokens"):
            self.dino_codec.num_prefix_tokens = int(n_prefix)

        decoded = self.dino_codec.decompress(**coded_unit)
        h_hat = decoded["h_hat"]
        if isinstance(h_hat, torch.Tensor) and h_hat.dim() == 3:
            n_expected = n_prefix + n_spatial
            if int(h_hat.shape[1]) != n_expected:
                raise RuntimeError(
